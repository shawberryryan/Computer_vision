...
def make_synthetic_view(img, corners, size):
    ...
    '''
    height = int(size[0][0] * 100)
    width = int(size[0][1] * 100)
    size_px = (width, height)
    dims = np.array([[0, 0], [width-1, 0], [width-1, height-1], [0, height-1]])

    # Compute H using fit_homography
    H = fit_homography(np.hstack((corners, dims)))

    # Use warpPerspective to transform the image
    synthetic_view = cv2.warpPerspective(img, H, size_px)
