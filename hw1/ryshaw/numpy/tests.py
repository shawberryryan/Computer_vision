...
def t9(q, k, v):
    ...
    """
    qk = -np.sum((q - k) ** 2, axis=-1, keepdims=True)
    resArr = np.exp(qk) * v
    return np.sum(resArr)
...
def t10(Xs):
    ...
    return np.sqrt(np.sum((centroids[:, None] - centroids) ** 2, axis=-1))
...
def t11(X):
    ...
    distances = xNorm + xNorm.T - 2*np.dot(X, X.T)
...
def t12(X, Y):
    ...
    return np.sqrt(np.maximum(xNorm + yNorm.T - 2*np.dot(X, Y.T), 0))
...
def t18(N, r, x, y):
    ...
    return np.less(np.sqrt((X - x)**2 + (Y - y)**2), r).astype(float)
...
def t19(N, s, x, y):
    ...
    """
    X, Y = np.meshgrid(np.arange(N), np.arange(N))
    return np.exp(-(np.square(X - x) + np.square(Y - y)) / np.square(s))
