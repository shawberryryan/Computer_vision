...
def resizeToSquare(I, maxDim):
    ...
    if height > width:
        I = cv2.resize(I, (maxDim, int(maxDim * height / width)))
    else:
        I = cv2.resize(I, (int(maxDim * width / height), maxDim))
    return I


def quantize(v, palette):
    """
    Given a scalar or vector v and array of values palette,
    return the index of the closest value(s)
    """
    if np.isscalar(v):
        return np.argmin(np.abs(palette - v))
    else:
        diff = np.abs(palette - v[:, np.newaxis])
...
def quantizeNaive(IF, palette):
    ...
    # quantizing multiple
    shape = IF.shape
    QF = np.zeros(shape, dtype=np.uint8)
    for i in range(shape[0]):
        for j in range(shape[1]):
            QF[i, j] = quantize(IF[i, j], palette)
    return QF
...
def quantizeFloyd(IF, palette):
    ...
    for i in range(height):
        for j in range(width):
            oldval = (IT[i,j])
            pixval=palette[quantize(oldval,palette)]
            output[i,j] = np.rint(blevels*pixval)
            error = oldval-pixval
            if(j+1<width):
                IT[i,j+1] += error*7/16
            if(i+1<height):
